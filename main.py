...
async def handle_review_response(rid: str, customer_phone: str, message_text: str) -> str | None:
    ...
    greeting_patterns = ["bonjour", "bonsoir", "salut", "hello", "hi", "hey", "coucou", "bonne soirée",
                         "good morning", "good evening", "buongiorno", "buonasera", "réserver", "reservation",
                         "menu", "carte", "horaire", "ouvert", "table", "disponible"]
    msg_lower = message_text.strip().lower()
    if any(msg_lower.startswith(g) or msg_lower == g for g in greeting_patterns):
    ...
    for rid, rq in review_queue.items():
        ...
        for r in rq:
            if r["sent"] or r.get("responded"):
                continue
            # Only send reviews for bookings scheduled today
            scheduled_at = r.get("scheduled_at", "")
            if scheduled_at:
                try:
                    scheduled_date = scheduled_at[:10]
                    if scheduled_date != today:
                        # Mark old reviews as sent to avoid spamming on restart
                        r["sent"] = True
                        continue
                except Exception:
                    pass
            ...
            if booking_time_str and ":" in booking_time_str:
                try:
                    ...
                    meal_dt = now.replace(hour=int(bh), minute=int(bm), second=0, microsecond=0)
                    send_after = meal_dt + timedelta(hours=2)
                    if now >= send_after:
                        await send_review_request(rid, r["phone"], r["name"])
                        r["sent"] = True
                except Exception as e:
                    logger.warning(f"Review timing error: {e}")
                    # Fallback: send 3h after scheduling
                    try:
                        ...
                        r["sent"] = True  # Give up, mark as sent
            else:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            if resp.status_code == 200:
                ...
                fallback_text = f"Bonjour ! Vous avez essaye de joindre {restaurant_name} et nous n'avons pas pu prendre votre appel. Je suis l'assistant du restaurant, comment puis-je vous aider ? Reservation, menu, horaires... je suis la pour vous !"
...
async def handle_missed_call(caller_phone: str, restaurant_phone: str):
    ...
    if success:
        ...
        save_message(rid, caller_normalized, "assistant",
            f"[Appel manque] Message automatique envoye suite a un appel sans reponse du restaurant {restaurant_name}.")
