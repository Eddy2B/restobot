...
function loadAll(){
  ...
  setInterval(function(){fetchDataSilent()},30000);
}
...
function fetchDataSilent(){
  ...
  Promise.all([
    ...
    if(res[3])floorplan=(res[3].tables||[]);
    if(res[4])reviewQueue=(res[4].queue||[]);
    if(res[5])dailyMsg=(res[5].message)||'';
    if(res[6])menuSections=(res[6].sections)||[];
    updateBadges();
    // Re-render current page if on overview
    if(currentPage==='overview')renderPage('overview');
  }).catch(function(){});
}
