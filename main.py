...
.lerr{color:var(--da);font-size:13px;margin-bottom:14px;display:none}
...
<div class="lbox">
  ...
  <div class="lcd">
    <div class="lerr" id="loginError">Mot de passe incorrect</div>
    <input class="linp" type="password" id="loginPwd" placeholder="Mot de passe" onkeydown="if(event.key==='Enter')doLogin()">
...
<div class="sidebar">
  ...
  <div class="sb-n">
    ...
    <button class="nb on" data-pg="overview" onclick="switchPage('overview',this)"><span class="ic">◎</span> Vue d'ensemble</button>
    <button class="nb" data-pg="floorplan" onclick="switchPage('floorplan',this)"><span class="ic">⊞</span> Plan de salle</button>
    <button class="nb" data-pg="bookings" onclick="switchPage('bookings',this)"><span class="ic">◉</span> Réservations <span class="nb-badge" id="bookBadge" style="background:var(--wa);color:#fff">0</span></button>
    <button class="nb" data-pg="menu" onclick="switchPage('menu',this)"><span class="ic">◐</span> Menu</button>
    <button class="nb" data-pg="conversations" onclick="switchPage('conversations',this)"><span class="ic">◈</span> Conversations <span class="nb-badge" id="convBadge" style="background:var(--ac);color:#fff">0</span></button>
    <button class="nb" data-pg="reviews" onclick="switchPage('reviews',this)"><span class="ic">★</span> Avis <span class="nb-badge" id="reviewBadge" style="background:var(--ac);color:#fff">0</span></button>
    <button class="nb" data-pg="contacts" onclick="switchPage('contacts',this)"><span class="ic">◇</span> Contacts</button>
    <button class="nb" data-pg="config" onclick="switchPage('config',this)"><span class="ic">⚙</span> Configuration</button>
    <button class="nb" data-pg="stats" onclick="switchPage('stats',this)"><span class="ic">◫</span> Statistiques</button>
  </div>
  <div class="sb-u">
...
<div class="mobile-nav">
  <div class="mobile-nav-items">
    <button class="mobile-nav-btn active" onclick="switchPage('overview',this)"><span>◎</span>Accueil</button>
    <button class="mobile-nav-btn" onclick="switchPage('floorplan',this)"><span>⊞</span>Plan</button>
    <button class="mobile-nav-btn" onclick="switchPage('bookings',this)"><span>◉</span>Résas</button>
    <button class="mobile-nav-btn" onclick="switchPage('conversations',this)"><span>◈</span>Chat</button>
    <button class="mobile-nav-btn" onclick="switchPage('config',this)"><span>⚙</span>Config</button>
  </div>
</div>
...
function doLogin(){if(document.getElementById('loginPwd').value===DP){document.getElementById('loginOverlay').style.display='none';document.getElementById('app').classList.add('v');loadAll()}else{document.getElementById('loginError').style.display='block'}}
...
function fetchData(){
  ...
  Promise.all([
    fetch('/api/bookings?key='+SK).then(r=>r.json()),
    fetch('/api/contacts?key='+SK).then(r=>r.json()),
    fetch('/api/conversations?key='+SK).then(r=>r.json()),
    fetch('/api/floorplan?key='+SK).then(r=>r.json()),
    fetch('/api/reviews?key='+SK).then(r=>r.json()),
    fetch('/api/config?key='+SK).then(r=>r.json()),
    fetch('/api/daily?key='+SK).then(r=>r.json()).catch(()=>({message:''}))
  ]).then(function(res){
    bookings=res[0]||[];
    contacts=res[1]||{};
    conversations=res[2]||{};
...
function renderOverview(c){
  ...
  if(overviewBlocks.daily){
    ...
    h+='<textarea class="dtx-edit" id="dailyEdit" style="display:none" onkeydown="if(event.key===\'Enter\'&&!event.shiftKey){event.preventDefault();saveDaily()}"></textarea>';
  ...
  if(overviewBlocks.stats){
    ...
    h+='<div class="sc"><div class="sl">Messages</div><div class="sv" style="color:var(--ac)">'+convArr.reduce(function(a,e){return a+(e[1].messages||[]).length},0)+'</div><div class="ss2">aujourd\'hui</div></div>';
  ...
  if(overviewBlocks.floor){
    h+='<div class="fm" id="ov-floor" onclick="switchPage(\'floorplan\')"><div style="display:flex;justify-content:space-between;align-items:center"><div><div class="card-t">Plan de salle</div><div class="card-s">'+floorplan.length+' tables · '+totalSeats+' places</div></div><span style="font-size:12px;color:var(--ac);font-weight:600">Modifier →</span></div><div class="fc" id="floorMiniCanvas"></div></div>';
  }
  ...
  if(overviewBlocks.bookings){
    ...
    convArr.slice(0,4).forEach(function(e,i){
      ...
      var name=data.name||phone;
      var lastMsg=(data.messages&&data.messages.length)?data.messages[data.messages.length-1]:{};
      var colors=['var(--al)','var(--blb)','var(--okb)','var(--wab)'];
      var tcolors=['var(--ac)','var(--bl2)','var(--ok)','var(--wa)'];
      h+='<div class="cr"><div class="cav" style="background:'+colors[i%4]+';color:'+tcolors[i%4]+'">'+name.charAt(0).toUpperCase()+'</div><div style="flex:1;min-width:0"><div style="display:flex;justify-content:space-between"><span style="font-size:14px;font-weight:600">'+name+'</span></div><div class="cmsg">'+(lastMsg.text||'...')+'</div></div></div>';
    });
...
function renderMenu(c){
  var menu=restaurantConfig.menu||'';
  var h='';
  // Daily message
  ...
  h+='<div class="card" style="padding:20px"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px"><div><div class="card-t">La Carte</div><div class="card-s">'+(restaurantConfig.name||'Restaurant')+'</div></div></div>';
  h+='<div style="white-space:pre-wrap;font-size:14px;line-height:1.8;color:var(--t)">'+(menu||'Menu non configuré. Allez dans Configuration pour ajouter votre menu.')+'</div>';
  h+='</div>';
  c.innerHTML=h;
}
...
function renderConversations(c){
  ...
  entries.forEach(function(e,i){
    var phone=e[0],data=e[1];
    var name=data.name||phone;
    var lastMsg=(data.messages&&data.messages.length)?data.messages[data.messages.length-1].text:'';
    h+='<div class="conv-list-item'+(i===0?' selected':'')+'" onclick="selectConv(\''+phone+'\',this)"><div style="font-size:13px;font-weight:600">'+name+'</div><div style="font-size:11px;color:var(--tm);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+lastMsg+'</div></div>';
  });
...
function selectConv(phone,el){
  ...
  (data.messages||[]).forEach(function(m){
    h+='<div class="bubble '+(m.role==='user'?'bubble-user':'bubble-bot')+'">'+m.text+'</div>';
  });
  mc.innerHTML=h;
...
function renderConfig(c){
  ...
  fields.forEach(function(f){
    h+='<div class="cfr"><div><div class="cfl">'+f.l+'</div><div class="cfd">'+(restaurantConfig[f.k]||'Non configuré')+'</div></div><span style="font-size:12px;color:var(--ac);font-weight:600;cursor:pointer" onclick="editConfigField(\''+f.k+'\',\''+f.l+'\')">Modifier</span></div>';
  });
  h+='</div></div>';
  
  // Menu config
  h+='<div class="card" style="padding:20px"><div class="cfs"><div class="cft">Menu</div><div class="cfsb">Le menu complet utilisé par l\'agent IA</div><textarea class="dinp" id="menuInput" style="min-height:200px" placeholder="Collez votre menu ici...">'+(restaurantConfig.menu||'')+'</textarea><button class="ba" style="margin-top:8px" onclick="saveMenu()">💾 Enregistrer le menu</button></div></div>';
...
function toggleOverviewBlock(el){
  ...
  showToast(el.classList.contains('on')?'Bloc activé':'Bloc masqué');
}
...
function saveMenu(){
  restaurantConfig.menu=document.getElementById('menuInput').value;
  fetch('/api/config?key='+SK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(restaurantConfig)});
...
function showResaTableSelect(){
  ...
  floorplan.forEach(function(t){
    ...
    h+='<div class="tsb'+(taken?' taken':t.id===resaSelTable?' sel':'')+'" '+(taken?'':'onclick="pickResaTable(\''+t.id+'\')"')+'>'+t.id+'<br><span style="font-size:10px;color:var(--tm)">'+t.seats+'p</span></div>';
  });
