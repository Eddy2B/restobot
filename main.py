...
RATE_LIMITS = {
    "/api/login": (5, 60),         # 5 attempts per 60 seconds
...
async def api_login(request: Request):
    ...
    if check_login_lockout(client_ip):
        return {"error": "Compte temporairement verrouillé suite à trop de tentatives. Réessayez plus tard."}
...
async def api_change_password(request: Request):
    ...
    if len(new_pwd) < 6:
        return {"error": "Minimum 12 caractères"}
    try:
        async with db_pool.acquire() as conn:
