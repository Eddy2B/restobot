...
function renderDashTable(){
  ...
  restaurants.forEach(function(r){
    ...
    h+='<button class="btn btn-ghost btn-xs" onclick="openDetail(\''+r.id+'\')">Détails</button>';
    h+='<button class="btn btn-ghost btn-xs" onclick="openEdit(\''+r.id+'\')">Modifier</button>';
    h+='<button class="btn btn-danger btn-xs" onclick="confirmDelete(\''+r.id+'\',\''+esc(r.name)+'\')">Suppr.</button>';
    h+='</div></td>';
    h+='</tr>';
  });
...
function renderRestoTable(){
  ...
  restaurants.forEach(function(r){
    ...
    h+='<td><div style="display:flex;gap:4px">';
    h+='<button class="btn btn-ghost btn-xs" onclick="openEdit(\''+r.id+'\')">Modifier</button>';
    h+='<button class="btn btn-ok btn-xs" onclick="setStatus(\''+r.id+'\',\'active\')">Activer</button>';
    h+='<button class="btn btn-danger btn-xs" onclick="setStatus(\''+r.id+'\',\'suspended\')">Suspendre</button>';
    h+='</div></td>';
    h+='</tr>';
  });
...
function renderEditTab(){
  ...
  if(currentTab==='general'){
    ...
    h+='<div class="form-group"><label>Ton de l\'IA</label><textarea id="ef-tone" rows="2">'+esc(s.tone||'')+'</textarea></div>';
    h+='<div class="form-row">';
...
function openDetail(rid){
  apiFetch('/api/admin/restaurant/'+rid).then(function(r){return r.json()}).then(function(d){
    ...
    h+='<button class="btn btn-primary btn-sm" onclick="closeDetail();openEdit(\''+rid+'\')">Modifier</button>';
...
function loadBookings(){
  ...
  apiFetch('/api/admin/restaurant/'+rid+'/bookings').then(function(r){return r.json()}).then(function(d){
    ...
    bks.forEach(function(b){
      ...
      h+='<td><button class="btn btn-danger btn-xs" onclick="deleteBooking(\''+rid+'\',\''+esc(b.id)+'\')">Suppr.</button></td>';
      h+='</tr>';
    });
...
function loadConversations(){
  ...
  apiFetch('/api/admin/restaurant/'+rid+'/conversations').then(function(r){return r.json()}).then(function(d){
    ...
    keys.forEach(function(phone){
      ...
      h+='<div class="card" style="cursor:pointer" onclick="this.querySelector(\'.conv-msgs\').style.display=this.querySelector(\'.conv-msgs\').style.display===\'block\'?\'none\':\'block\'">';
