...
IMPORTANT : Si le client demande si le restaurant est ouvert, vérifie l'heure actuelle par rapport aux horaires. Si l'heure actuelle dépasse l'heure de fermeture, informe que le restaurant est fermé pour ce service.
...
async def admin_update_restaurant(rid: str, request: Request):
    ...
    return {"status": "ok", "restaurant": rest}
...
function renderKPIs(d){
  ...
  h+=kpi(d.total_bookings,'Réservations','var(--ac)');
...
function renderDashTable(){
  ...
  restaurants.forEach(function(r){
    ...
    h+='<button class="btn btn-ghost btn-xs" onclick="openDetail(\x27'+r.id+'\x27)">Détails</button>';
    h+='<button class="btn btn-ghost btn-xs" onclick="openEdit(\x27'+r.id+'\x27)">Modifier</button>';
    h+='<button class="btn btn-danger btn-xs" onclick="confirmDelete(\x27'+r.id+'\x27,\x27'+esc(r.name)+'\x27)">Suppr.</button>';
    h+='</div></td>';
    h+='</tr>';
  });
...
function renderRestoTable(){
  ...
  restaurants.forEach(function(r){
    ...
    h+='<td><div style="display:flex;gap:4px">';
    h+='<button class="btn btn-ghost btn-xs" onclick="openEdit(\x27'+r.id+'\x27)">Modifier</button>';
    h+='<button class="btn btn-ok btn-xs" onclick="setStatus(\x27'+r.id+'\x27,\x27active\x27)">Activer</button>';
    h+='<button class="btn btn-danger btn-xs" onclick="setStatus(\x27'+r.id+'\x27,\x27suspended\x27)">Suspendre</button>';
    h+='</div></td>';
    h+='</tr>';
  });
...
function renderEditTab(){
  ...
  if(currentTab==='general'){
    ...
    h+='<div class="form-group"><label>Ton de l\x27IA</label><textarea id="ef-tone" rows="2">'+esc(s.tone||'')+'</textarea></div>';
    h+='<div class="form-row">';
...
function openDetail(rid){
  apiFetch('/api/admin/restaurant/'+rid).then(function(r){return r.json()}).then(function(d){
    ...
    h+='<button class="btn btn-primary btn-sm" onclick="closeDetail();openEdit(\x27'+rid+'\x27)">Modifier</button>';
...
function loadBookings(){
  var rid=document.getElementById('bookingRestoFilter').value;
  if(!rid && restaurants.length) rid=restaurants[0].id;
  if(!rid)return;
  apiFetch('/api/admin/restaurant/'+rid+'/bookings').then(function(r){return r.json()}).then(function(d){
    ...
    bks.forEach(function(b){
      ...
      h+='<td><button class="btn btn-danger btn-xs" onclick="deleteBooking(\x27'+rid+'\x27,\x27'+esc(b.id)+'\x27)">Suppr.</button></td>';
      h+='</tr>';
    });
...
function loadConversations(){
  var rid=document.getElementById('convRestoFilter').value;
  if(!rid && restaurants.length) rid=restaurants[0].id;
  if(!rid)return;
  apiFetch('/api/admin/restaurant/'+rid+'/conversations').then(function(r){return r.json()}).then(function(d){
    ...
    keys.forEach(function(phone){
      ...
      h+='<div class="card" style="cursor:pointer" onclick="this.querySelector(\x27.conv-msgs\x27).style.display=this.querySelector(\x27.conv-msgs\x27).style.display===\x27block\x27?\x27none\x27:\x27block\x27">';
...
function confirmDelete(rid,name){
  ...
  document.getElementById('confirmText').textContent='Cette action est irréversible. Toutes les données seront supprimées.';
