...
const titles={floorplan:"Plan de salle",bookings:"Reservations",conversations:"Conversations",reviews:"Avis Google",dashboard:"Statistiques"};
function switchPage(id,btn){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+id).classList.add('active');document.querySelectorAll('.nav-item').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');document.getElementById('pageTitle').textContent=titles[id]||id;if(id==='conversations')fetchConversations();if(id==='reviews')fetchReviews();}
...
function loadAll(){fetchFloorplan();fetchDashboard();fetchAllBookings();fetchConversations();fetchReviews();}
