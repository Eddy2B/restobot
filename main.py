...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .login-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: #0A1628; display: flex; justify-content: center; align-items: center; z-index: 1000; }
        .login-box { background: #1A2744; padding: 40px; border-radius: 16px; width: 380px; text-align: center; border: 1px solid #2A3A55; }
        .login-box h2 { color: #C9A55C; font-size: 28px; margin-bottom: 8px; }
        .login-box p { color: #8899AA; margin-bottom: 24px; font-size: 14px; }
        .login-box input { background: #0D1E38; border: 1px solid #2A3A55; border-radius: 8px; padding: 12px 16px; color: white; font-size: 15px; width: 100%; margin-bottom: 16px; }
        .login-box input:focus { outline: none; border-color: #C9A55C; }
        .login-box button { background: #C9A55C; color: #0A1628; border: none; border-radius: 8px; padding: 12px; width: 100%; font-size: 15px; font-weight: 700; cursor: pointer; }
        .login-box button:hover { background: #D4B56A; }
...
<body>
    ...
    <div class="login-overlay" id="loginOverlay">
        <div class="login-box">
            <h2>🤖 RestoBot</h2>
            <p>Entrez votre mot de passe pour accéder au dashboard</p>
            <div class="login-error" id="loginError">Mot de passe incorrect</div>
            <input type="password" id="loginPassword" placeholder="Mot de passe" onkeyup="if(event.key==='Enter') doLogin()">
    ...
    <div class="main-content" id="mainContent">
        <div class="header">
            ...
            <div style="display:flex;align-items:center;gap:15px;">
                ...
                <button class="btn" style="background:#2A3A55;color:#8899AA;font-size:12px;padding:6px 12px;" onclick="doLogout()">Déconnexion</button>
            </div>
        </div>
        <div class="container">
            <div class="grid">
    ...
    <script>
        ...
        const CORRECT_PWD = '{{DASHBOARD_PASSWORD}}';
        ...
        async function apiPost(endpoint, data) {
            return fetch(BASE + endpoint + '?key=' + SECRET, {
                method: 'POST',
        ...
        async function setStatus(status) { await apiPost('/api/status', {status}); showToast(); fetchData(); }
        ...
        async function addClosed() {
            ...
            if (!d) return;
            await apiPost('/api/closed', {date: d, type: 'closed'});
        ...
        async function addFull() {
            const d = document.getElementById('closedDate').value;
            if (!d) return;
            await apiPost('/api/closed', {date: d, type: 'full'});
