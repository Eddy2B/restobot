...
function renderWaitlist(c){
  ...
  if(!active.length){
    ...
    active.forEach(function(w,i){
      ...
      if(w.status==='waiting')h+='<button style="padding:4px 10px;border-radius:6px;border:1px solid var(--ac);background:var(--al);color:var(--ac);font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f)" onclick="notifyWaitlist(\''+w.id+'\',\''+w.date+'\',\''+w.service+'\',\''+w.covers+'\')">Notifier</button>';
      h+='<button style="padding:4px 10px;border-radius:6px;border:1px solid var(--b);background:var(--card);color:var(--da);font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f)" onclick="removeWaitlist(\''+w.id+'\')">Retirer</button>';
      h+='</div></div>';
    });
  }
  h+='</div>';
