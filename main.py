...
def build_availability_context(phone_number_id: str) -> str:
    ...
    lines = [f"\n📅 DISPONIBILITÉS POUR AUJOURD'HUI ({date.today().strftime('%A %d %B').lower()}) :"]
...
def extract_booking_date(message: str) -> str:
    """Extract a booking date from a WhatsApp message. Returns YYYY-MM-DD string."""
    import re
    import locale
    msg = message.lower().strip()
    today = date.today()

    # Explicit date patterns: 15/03, 15 mars, 15 march, etc.
    ...
    # Relative days
    days_fr = {"aujourd'hui": 0, "aujourdhui": 0, "aujourd hui": 0, "today": 0,
               "demain": 1, "tomorrow": 1, "apres-demain": 2, "après-demain": 2, "après demain": 2, "apres demain": 2}
    for pattern, delta in days_fr.items():
        if pattern in msg:
            return (today + timedelta(days=delta)).isoformat()

    # "ce soir" / "ce midi" = today
    if "ce soir" in msg or "ce midi" in msg or "tonight" in msg or "this evening" in msg:
        return today.isoformat()

    # Day of week: lundi, mardi, etc.
    days_of_week_fr = {"lundi": 0, "mardi": 1, "mercredi": 2, "jeudi": 3, "vendredi": 4, "samedi": 5, "dimanche": 6}
    days_of_week_en = {"monday": 0, "tuesday": 1, "wednesday": 2, "thursday": 3, "friday": 4, "saturday": 5, "sunday": 6}
    all_days = {**days_of_week_fr, **days_of_week_en}
    ...
    for day_name, day_num in all_days.items():
        if day_name in msg:
            ...
            if delta == 0:
                # Check if "prochain" is mentioned
                if "prochain" in msg or "next" in msg:
                    delta = 7
                else:
                    delta = 0  # today if same day
            return (today + timedelta(days=delta)).isoformat()

    # "ce week-end" / "weekend"
    if "week-end" in msg or "weekend" in msg:
        current_day = today.weekday()
        delta = (5 - current_day) % 7  # next Saturday
        if delta == 0:
            delta = 0  # it's Saturday
        return (today + timedelta(days=delta)).isoformat()
...
📆 NOUS SOMMES LE : {date.today().strftime('%A %d %B %Y')}
...
def save_daily_stats_snapshot(phone_number_id: str, st: dict):
    """Save a day's stats to history."""
    snapshot_date = st.get("last_reset", date.today().isoformat())
    # Calculate from bookings
    day_bookings = [b for b in bookings if (b.get("date") or "").startswith(snapshot_date)]
    sources = {}
    total_covers = 0
    ...
    # Keep max 90 days
    if len(daily_stats_history) > 90:
        daily_stats_history.pop(0)
    logger.info(f"📊 Daily stats saved for {snapshot_date}: {len(day_bookings)} bookings, {total_covers} covers")


def build_daily_recap(phone_number_id: str) -> str:
    """Build the daily recap message for the owner."""
    today_str = date.today().isoformat()
    tomorrow_str = (date.today() + timedelta(days=1)).isoformat()

    today_bookings = [b for b in bookings if (b.get("date") or "").startswith(today_str)]
    tomorrow_bookings = [b for b in bookings if (b.get("date") or "").startswith(tomorrow_str)]
    ...
    total_tables = len(floor_tables.get(phone_number_id, []))
    occupied = len([b for b in today_bookings if b.get("table")])
    occ_rate = round(occupied / total_tables * 100) if total_tables else 0
    new_contacts_today = sum(1 for p, c in contacts.items() if (c.get("first_seen") or "").startswith(today_str))
    conv_count = len(conversations)
    st = stats.get(phone_number_id, {})
    msgs_today = st.get("messages_today", 0)
    pending_reviews = sum(1 for r in review_queue if not r.get("sent"))

    lines = [
        f"📊 *Recap du {date.today().strftime('%A %d %B')}*",
        "",
        f"🍽 *{len(today_bookings)} reservations* · {total_covers_today} couverts",
        f"📈 Taux occupation : {occ_rate}% ({occupied}/{total_tables} tables)",
        f"💬 {msgs_today} messages · {conv_count} conversations",
    ]
...
async def send_daily_recap():
    """Send daily recap to all restaurant owners at 22h."""
    for pid, restaurant in restaurants.items():
        owner = restaurant.get("owner_phone")
        if not owner:
            continue
        try:
            recap = build_daily_recap(pid)
            await send_whatsapp_message(pid, restaurant["access_token"], owner, recap)
            # Also save snapshot
            st = stats.get(pid, {})
            save_daily_stats_snapshot(pid, st)
            logger.info(f"📊 Daily recap sent to owner {owner}")
        except Exception as e:
...
async def notify_owner(restaurant: dict, customer_phone: str, customer_name: str, message: str):
    ...
    if is_booking:
        ...
        # Extract booking date from message
        booking_date = extract_booking_date(message)

        # Auto assign table if time found (only for today's bookings)
        assigned_table = None
        pid = restaurant["phone_number_id"]
        logger.info(f"🔍 Booking attempt: date={booking_date}, time={booking_time}, covers={covers}, pid={pid}")
        logger.info(f"🔍 Tables loaded: {len(floor_tables.get(pid, []))}, Slots initialized: {bool(table_slots.get(pid))}")
        
        is_today_booking = booking_date == date.today().isoformat()
        ...
        logger.info(f"🍽️ Booking {booking_id}: {customer_name} {covers}p @ {booking_date} {booking_time} -> {assigned_table or 'unassigned'}")
    ...
    if is_booking:
        ...
        try:
            ...
            elif bd == date.today() + timedelta(days=1):
                date_label = "demain"
            else:
...
function checkOnboarding(){
  ...
  fetch('/api/settings?key='+SK).then(function(r){return r.json()}).then(function(d){
...
function renderOnboarding(){
  ...
  // Logo
  h+='<div style="display:flex;align-items:center;gap:10px;margin-bottom:6px"><div style="width:36px;height:36px;background:var(--acg);border-radius:9px;display:flex;align-items:center;justify-content:center"><svg viewBox="0 0 24 24" style="width:20px;height:20px;fill:none;stroke:#fff;stroke-width:2"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke-linejoin="round" stroke-linecap="round"/></svg></div><div style="font-size:18px;font-weight:800;color:var(--t);letter-spacing:-.02em">GuestScale</div></div>';
...
function obFinish(){
  ...
  fetch('/api/config?key='+SK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cfg)}).then(function(){
    // Update local config
    for(var k in cfg){restaurantConfig[k]=cfg[k]}
    // Mark onboarding done
    try{sessionStorage.setItem('ob_done','1')}catch(e){}
    fetch('/api/settings?key='+SK+'&set=onboarding_done&value=1');
    // Hide overlay
...
function renderStats(c){
  ...
  fetch('/api/stats/history?key='+SK).then(function(r){return r.json()}).then(function(data){
    var t=data.today||{};
...
async def lifespan(app: FastAPI):
    ...
    async def recap_loop():
        while True:
            try:
                now = datetime.utcnow()
                # Send recap at 22:00 local (adjust for timezone if needed)
                if now.hour == 21 and now.minute < 6:
                    await send_daily_recap()
                    await asyncio.sleep(3600)  # Wait 1h to avoid double send
                else:
                    await asyncio.sleep(300)
            except Exception as e:
...
async def get_stats_history(request: Request):
    ...
    for b in today_bookings:
        s = b.get("source", "autre")
        sources_today[s] = sources_today.get(s, 0) + 1

    total_tables = len(floor_tables.get(pid, []))
    occupied = len([b for b in today_bookings if b.get("table")])
    st = stats.get(pid, {})
    ...
    today_data = {
        ...
        "new_contacts": sum(1 for p, c in contacts.items() if (c.get("first_seen") or "").startswith(today_str)),
        "pending_reviews": sum(1 for r in review_queue if not r.get("sent")),
    }

    # Build last 14 days from bookings data (even if no history snapshots)
    history = list(daily_stats_history)
    # Fill gaps from bookings
    for i in range(14, 0, -1):
        d = (date.today() - timedelta(days=i)).isoformat()
        if not any(h["date"] == d for h in history):
            day_bk = [b for b in bookings if (b.get("date") or "").startswith(d)]
            if day_bk:
                src = {}
                for b in day_bk:
                    s = b.get("source", "autre")
                ...
                history.append({
                    "date": d,
                    "bookings": len(day_bk),
                    "covers": sum(b.get("covers", 0) for b in day_bk),
                    "messages": 0,
                    "sources": src,
                })
...
async def get_stats_history(request: Request):
    ...
    today_bookings = [b for b in bookings if (b.get("date") or "").startswith(today_str)]
    tomorrow_bookings = [b for b in bookings if (b.get("date") or "").startswith(tomorrow_str)]
    total_covers = sum(b.get("covers", 0) for b in today_bookings)
    total_tables = len(floor_tables.get(pid, []))
    occupied = len([b for b in today_bookings if b.get("table")])
    occ_rate = round(occupied / total_tables * 100) if total_tables else 0
    st = stats.get(pid, {})
    ...
    return {"history": daily_stats_history[-30:], "today": today_data}
