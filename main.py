...
def build_availability_context(rid: str) -> str:
    ...
    lines.append("- RÈGLE STRICTE : Si MIDI est COMPLET et le client veut réserver le midi aujourd'hui, tu DOIS dire que c'est complet et proposer un autre créneau ou un autre jour. NE CONFIRME PAS de réservation.")
    lines.append("- RÈGLE STRICTE : Si SOIR est COMPLET et le client veut réserver le soir aujourd'hui, tu DOIS dire que c'est complet et proposer un autre créneau ou un autre jour. NE CONFIRME PAS de réservation.")
    lines.append("- Si le créneau demandé est complet AUJOURD'HUI, propose les créneaux les plus proches disponibles OU propose la liste d'attente OU propose de réserver un autre jour.")
    ...
    lines.append("- NE JAMAIS confirmer une réservation si aucun créneau n'est disponible pour la période demandée.")
    return "\n".join(lines)
