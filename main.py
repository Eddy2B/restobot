...
<div class="lbox">
  ...
  <div class="lcd">
    ...
    <div style="position:relative">
      ...
      <button onclick="togglePwdVis()" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:16px;color:#6B7280;padding:4px" id="pwdToggle" title="Afficher le mot de passe">&#128065;</button>
    </div>
...
function fetchDataSilent(){
  ...
  Promise.all([
    ...
    updateBadges();
    if(currentPage==='overview'||currentPage==='bookings'||currentPage==='conversations')renderPage(currentPage);
  }).catch(function(){});
}
...
function renderOverview(c){
  ...
  if(overviewBlocks.stats){
    ...
    h+='<div class="sc" data-nav="bookings" style="cursor:pointer"><div class="sl">Réservations</div><div class="sv" style="color:var(--ok)">'+tb.length+'</div><div class="ss2">aujourd&#39;hui</div></div>';
...
function renderFloorplan(c){
  ...
  var h='<div class="card" style="padding:20px;margin-bottom:14px">';
  // Header with mode toggle
  h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">';
  h+='<div><div class="card-t">Plan de salle</div><div class="card-s" id="fpSummary">'+floorplan.length+' tables · '+totalSeats+' places · <span style="color:var(--ok)">'+free+' libres</span> · <span style="color:var(--da)">'+booked+' occupees</span></div></div>';
  h+='<div style="display:flex;gap:6px">';
  h+='<button style="padding:8px 16px;border-radius:8px;border:1.5px solid '+(fpMode==='resa'?'var(--ac)':'var(--b)')+';background:'+(fpMode==='resa'?'var(--al)':'var(--card)')+';color:'+(fpMode==='resa'?'var(--ac)':'var(--ts)')+';font-size:12px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpModeResa>Reservations</button>';
  h+='<button style="padding:8px 16px;border-radius:8px;border:1.5px solid '+(fpMode==='edit'?'var(--ac)':'var(--b)')+';background:'+(fpMode==='edit'?'var(--al)':'var(--card)')+';color:'+(fpMode==='edit'?'var(--ac)':'var(--ts)')+';font-size:12px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpModeEdit>Modifier plan</button>';
  h+='</div></div>';
  ...
  if(fpMode==='edit'){
    ...
    var presets=[{s:'round',n:2},{s:'round',n:4},{s:'round',n:6},{s:'rect',n:2},{s:'rect',n:4},{s:'rect',n:6},{s:'rect',n:8}];
    presets.forEach(function(p){
      h+='<button style="padding:5px 10px;border-radius:7px;border:1.5px solid var(--b);background:var(--card);color:var(--t);font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f);white-space:nowrap;display:flex;align-items:center;gap:3px" data-fpAdd="'+p.s+'-'+p.n+'">';
      h+='<span style="width:'+(p.s==='round'?12:16)+'px;height:12px;border-radius:'+(p.s==='round'?'50%':'2px')+';border:2px solid var(--ac);display:inline-block"></span>'+p.n+'p</button>';
    });
    h+='<div style="margin-left:auto"><button class="ba" data-fpSave>Enregistrer le plan</button></div>';
    h+='</div>';
  }

  // Resa mode: time filter
  if(fpMode==='resa'){
    h+='<div style="display:flex;gap:6px;margin-bottom:10px;align-items:center">';
    h+='<span style="font-size:11px;font-weight:700;color:var(--tm)">Creneau :</span>';
    h+='<select id="fpTimeSlot" style="padding:6px 12px;border-radius:8px;border:1.5px solid var(--b);background:var(--card);font-size:12px;font-weight:600;cursor:pointer;font-family:var(--f);color:var(--t)">';
    h+='<option value="all">Tous</option>';
    var slots=["12:00","12:30","13:00","13:30","14:00","19:00","19:30","20:00","20:30","21:00","21:30","22:00"];
    slots.forEach(function(s){h+='<option value="'+s+'">'+s+'</option>'});
    h+='</select>';
    h+='<span style="margin-left:auto;font-size:12px;color:var(--tm)">Cliquez sur une table pour reserver ou voir la reservation</span>';
    h+='</div>';
  }
  ...
  fpZones.forEach(function(z,i){
    var xMin=i===0?0:i===1?46:84;
    var xMax=i===0?46:i===1?84:100;
    h+='<div style="position:absolute;left:'+xMin+'%;top:0;width:'+(xMax-xMin)+'%;height:100%;pointer-events:none">';
    h+='<div style="position:absolute;top:10px;left:50%;transform:translateX(-50%);font-size:10px;color:var(--tm);font-weight:700;letter-spacing:.06em;white-space:nowrap">'+z.label.toUpperCase()+'</div>';
    if(i<fpZones.length-1) h+='<div style="position:absolute;right:0;top:0;bottom:0;width:1px;border-right:1px dashed var(--b)"></div>';
    h+='</div>';
  });
  h+='</div>';

  // Edit mode: table editor panel
  if(fpMode==='edit'){
    h+='<div id="fpEditor" style="display:none;margin-top:12px;padding:16px;background:var(--card);border:1px solid var(--b);border-radius:12px">';
    h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div style="font-size:15px;font-weight:700" id="fpEdTitle">Table</div><button style="padding:4px 10px;border-radius:6px;border:none;background:#EF444415;color:#EF4444;font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpDel>Supprimer</button></div>';
    h+='<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px">';
    h+='<div><div class="finp-label">Nom</div><input class="finp" id="fpEdName" style="margin-bottom:0" placeholder="T1"></div>';
    h+='<div><div class="finp-label">Places</div><select class="finp" id="fpEdSeats" style="margin-bottom:0;cursor:pointer"><option>2</option><option>4</option><option>6</option><option>8</option><option>10</option><option>12</option></select></div>';
    h+='<div><div class="finp-label">Forme</div><select class="finp" id="fpEdShape" style="margin-bottom:0;cursor:pointer"><option value="round">Ronde</option><option value="rect">Rectangle</option></select></div>';
    h+='</div>';
    h+='<div style="margin-top:10px"><div class="finp-label">Zone</div><div style="display:flex;gap:4px" id="fpEdZones"></div></div>';
    h+='</div>';
  }

  // Resa mode: table info popup
  if(fpMode==='resa'){
    h+='<div id="fpResaPopup" style="display:none;margin-top:12px;padding:16px;background:var(--card);border:1px solid var(--b);border-radius:12px">';
    h+='<div id="fpResaContent"></div>';
    h+='</div>';
  }
...
function fpShowResaInfo(idx){
  ...
  var bk=null;
  bookings.forEach(function(b){if(b.table===t.id)bk=b});
  if(!bk){popup.style.display='none';return}
  popup.style.display='block';
  var h='<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">';
  h+='<div><div style="font-size:16px;font-weight:700">'+bk.name+'</div>';
  h+='<div style="font-size:12px;color:var(--tm);margin-top:2px">'+bk.covers+'p · '+(bk.booking_time||bk.time||'')+' · Table '+t.id+' ('+t.seats+'p, '+t.zone+')</div>';
  if(bk.phone)h+='<div style="font-size:12px;color:var(--ts);margin-top:2px">'+bk.phone+'</div>';
  h+='</div>';
  h+='<button style="padding:4px 10px;border-radius:6px;border:none;background:var(--bg);color:var(--ts);font-size:11px;font-weight:600;cursor:pointer;font-family:var(--f)" data-fpClosePopup>Fermer</button>';
  h+='</div>';
  // Change table selector
  h+='<div class="finp-label">Deplacer vers une autre table</div>';
  h+='<div style="display:flex;gap:4px;flex-wrap:wrap">';
  floorplan.forEach(function(ot,oi){
    if(oi===idx)return;
    var otBooked=ot.booking_name;
    var color=otBooked?'#F59E0B':'#10B981';
    h+='<button style="padding:5px 10px;border-radius:6px;border:1.5px solid '+color+'40;background:'+color+'08;color:'+color+';font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpSwap="'+bk.id+'-'+ot.id+'" title="'+(otBooked?'Swap avec '+otBooked:'Libre')+'">'+ot.id+' ('+ot.seats+'p)</button>';
  });
  h+='</div>';
...
function fpSwapTable(bookingId,newTableId){
  ...
  // Move this booking to new table
  fetch('/api/bookings/update?key='+SK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({booking_id:bookingId,table:newTableId})}).then(function(){
    fetchData();
  ...
  mergeBookingsIntoFloor();
  fpDrawTables();
  fpShowResaInfo(floorplan.findIndex(function(t){return t.id===newTableId}));
}
...
function renderBookings(c){
  ...
  var h='<div class="card"><div class="card-h"><div><div class="card-t">Toutes les reservations</div><div class="card-s">'+bookings.length+' au total</div></div><button class="ba" onclick="openResaModal()">+ Nouvelle</button></div>';
  bookings.forEach(function(b,i){
    h+='<div class="rw" data-editResa="'+i+'" style="cursor:pointer"><div class="rl"><div class="dot" style="background:'+(srcColors[b.source]||'#A8A29E')+'"></div><div><div style="font-size:14px;font-weight:600">'+b.name+'</div><div style="font-size:12px;color:var(--tm)">'+b.covers+'p · '+(b.date||'')+' '+(b.booking_time||b.time||'')+(b.phone?' · '+b.phone:'')+'</div></div></div><div style="display:flex;align-items:center;gap:6px"><span class="src-badge" style="color:'+(srcColors[b.source]||'#A8A29E')+';background:'+(srcColors[b.source]||'#A8A29E')+'15">'+(srcLabels[b.source]||b.source)+'</span><span class="badge" style="background:var(--okb);color:var(--ok)">'+(b.table||'—')+'</span></div></div>';
  });
  if(!bookings.length) h+='<div style="padding:30px;text-align:center;color:var(--tm)">Aucune reservation</div>';
  h+='</div>';
  c.innerHTML=h;
}
...
function renderStats(c){
  ...
  h+='<div class="sc"><div class="sl">Reservations</div><div class="sv" style="color:var(--ok)">'+totalResas+'</div><div class="ss2">total</div></div>';
  ...
  h+='<div class="sc"><div class="sl">Contacts</div><div class="sv" style="color:var(--wa)">'+totalContacts+'</div><div class="ss2">en base CRM</div></div>';
  h+='</div>';

  // Sources chart
  h+='<div class="card" style="padding:20px;margin-bottom:16px"><div class="card-t" style="margin-bottom:16px">Sources de reservation</div>';
  ...
  if(srcEntries.length){
    srcEntries.forEach(function(e){
      ...
      var col=srcColors[e[0]]||'#A8A29E';
      h+='<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">';
      h+='<div style="width:80px;font-size:12px;font-weight:600;color:var(--ts)">'+(srcLabels[e[0]]||e[0])+'</div>';
      h+='<div style="flex:1;height:24px;background:var(--bg);border-radius:6px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:'+col+';border-radius:6px;transition:width .3s"></div></div>';
      h+='<div style="width:40px;text-align:right;font-size:13px;font-weight:700;color:var(--t)">'+e[1]+'</div>';
      h+='</div>';
    });
  }else{
  ...
  // Conversations
  h+='<div class="card" style="padding:20px"><div class="card-t" style="margin-bottom:8px">Conversations</div>';
  h+='<div class="card-s" style="margin-bottom:16px">'+convArr.length+' clients actifs · '+totalMsgs+' messages echanges</div>';
  h+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">';
  h+='<div style="padding:16px;background:var(--bg);border-radius:10px;text-align:center"><div style="font-size:24px;font-weight:800;color:var(--ac)">'+totalMsgs+'</div><div style="font-size:11px;color:var(--tm);font-weight:600">Messages total</div></div>';
  h+='<div style="padding:16px;background:var(--bg);border-radius:10px;text-align:center"><div style="font-size:24px;font-weight:800;color:var(--ok)">'+(totalMsgs&&convArr.length?Math.round(totalMsgs/convArr.length):0)+'</div><div style="font-size:11px;color:var(--tm);font-weight:600">Moy par client</div></div>';
  h+='</div></div>';
...
document.addEventListener('click',function(e){
  ...
  if(e.target.closest('[data-fpModeEdit]')){fpMode='edit';renderFloorplan(document.getElementById('mainContent'));return}
