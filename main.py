...
<div class="sidebar">
  ...
  <div style="padding:0 12px;flex:1">
    ...
    <button class="nav-item" onclick="switchPage('config',this)">⚙️ Configuration</button>
      ...
      <div style="display:grid;grid-template-columns:1fr 340px;gap:0">
        ...
        <div style="background:white;border-left:1px solid #E2E8F0;border-radius:0 14px 14px 0;overflow-y:auto;max-height:520px;box-shadow:0 1px 3px rgba(0,0,0,.04)">
          <div style="padding:16px 20px;border-bottom:1px solid #E2E8F0">
            <div style="display:flex;justify-content:space-between;align-items:center"><div style="font-size:15px;font-weight:700;color:#0F1B2D" id="fpPanelTitle">Reservations</div><button onclick="showAddBooking()" style="background:#00D4AA;color:white;border:none;border-radius:8px;padding:6px 12px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">+ Nouvelle</button></div>
    ...
    <div class="page" id="page-config">
      <div style="max-width:700px">
        <div class="card" style="padding:24px;margin-bottom:16px">
          <div class="card-title" style="margin-bottom:20px">Informations du restaurant</div>
          <div class="stat-label">NOM</div>
          <input class="msg-input" type="text" id="cfg-name" placeholder="Nom du restaurant" style="margin-bottom:12px">
          <div class="stat-label">ADRESSE</div>
          <input class="msg-input" type="text" id="cfg-address" placeholder="Adresse" style="margin-bottom:12px">
          <div class="stat-label">TELEPHONE</div>
          <input class="msg-input" type="text" id="cfg-phone" placeholder="04 93 XX XX XX" style="margin-bottom:12px">
          <div class="stat-label">HORAIRES</div>
          <input class="msg-input" type="text" id="cfg-hours" placeholder="Mar-Sam 12h-14h 19h-22h30" style="margin-bottom:12px">
          <div class="stat-label">DESCRIPTION</div>
          <textarea id="cfg-description" placeholder="Restaurant italien au Port de Nice..." style="width:100%;padding:11px 14px;border-radius:10px;background:#F8FAFC;border:1.5px solid #E2E8F0;color:#0F1B2D;font-size:13px;outline:none;font-family:inherit;height:80px;resize:vertical;margin-bottom:12px"></textarea>
          <div class="stat-label">TON DE L'AGENT IA</div>
          <input class="msg-input" type="text" id="cfg-tone" placeholder="Chaleureux et professionnel" style="margin-bottom:12px">
          <div class="stat-label">LANGUES</div>
          <input class="msg-input" type="text" id="cfg-languages" placeholder="francais, anglais, italien" style="margin-bottom:12px">
          <div class="stat-label">INFOS SPECIALES</div>
          <input class="msg-input" type="text" id="cfg-special" placeholder="Brunch dimanche, soiree truffe..." style="margin-bottom:4px">
        </div>

        <div class="card" style="padding:24px;margin-bottom:16px">
          <div class="card-title" style="margin-bottom:4px">Carte / Menu</div>
          <div class="card-sub" style="margin-bottom:16px">L'agent IA utilise ces infos pour repondre aux clients</div>
          <textarea id="cfg-menu" placeholder="ANTIPASTI&#10;Burrata 14€&#10;Carpaccio 16€&#10;&#10;PRIMI&#10;Linguine vongole 18€&#10;..." style="width:100%;padding:14px;border-radius:10px;background:#F8FAFC;border:1.5px solid #E2E8F0;color:#0F1B2D;font-size:13px;outline:none;font-family:inherit;height:250px;resize:vertical;margin-bottom:12px;line-height:1.6"></textarea>
          <div class="stat-label">POLITIQUE ALLERGENES</div>
          <input class="msg-input" type="text" id="cfg-allergens" placeholder="Nous adaptons nos plats aux allergies" style="margin-bottom:4px">
        </div>

        <div class="card" style="padding:24px;margin-bottom:16px">
          <div class="card-title" style="margin-bottom:16px">Pages du dashboard</div>
          <div id="cfgPages" style="display:grid;grid-template-columns:1fr 1fr;gap:8px"></div>
        </div>

        <button onclick="saveConfig()" style="width:100%;padding:14px;border-radius:12px;border:none;background:#00D4AA;color:white;font-size:15px;font-weight:700;cursor:pointer;font-family:inherit;margin-bottom:16px">💾 Enregistrer les modifications</button>
        <div class="card" style="padding:24px;margin-bottom:16px">
          <div class="card-title" style="margin-bottom:4px">💬 Widget Chat Web</div>
          <div class="card-sub" style="margin-bottom:16px">Installez le chat IA sur votre site internet</div>
          <div class="stat-label">CODE A COPIER</div>
          <div style="position:relative">
            <textarea id="cfg-widget-code" readonly style="width:100%;padding:14px;border-radius:10px;background:#0F1B2D;border:none;color:#00D4AA;font-size:12px;font-family:monospace;height:60px;resize:none;line-height:1.6"></textarea>
            <button onclick="copyWidgetCode()" style="position:absolute;top:8px;right:8px;background:#00D4AA;color:white;border:none;border-radius:6px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">Copier</button>
          </div>
          <div style="margin-top:12px;display:flex;gap:8px;align-items:center">
            <div class="stat-label" style="margin:0">COULEUR</div>
            <input type="color" id="cfg-widget-color" value="#00D4AA" onchange="updateWidgetCode()" style="width:36px;height:28px;border:none;border-radius:4px;cursor:pointer">
            <a id="cfg-widget-preview" href="/widget-preview" target="_blank" style="margin-left:auto;color:#2563EB;font-size:12px;font-weight:600;text-decoration:none">Apercu en direct →</a>
          </div>
        </div>

        <div style="background:rgba(37,99,235,.06);border:1px solid rgba(37,99,235,.15);border-radius:12px;padding:16px">
          <div style="font-size:13px;font-weight:600;color:#2563EB;margin-bottom:6px">💡 Mise a jour instantanee</div>
          <div style="font-size:12px;color:#64748B;line-height:1.6">Les modifications sont appliquees immediatement. L'agent IA utilisera les nouvelles infos des le prochain message client.</div>
        </div>
      </div>
    </div>
...
<div class="mobile-nav" id="mobileNav">
  <div class="mobile-nav-items">
    ...
    <button class="mobile-nav-btn" onclick="switchPage('config',null);setMobileActive(this)"><span>⚙️</span>Config</button>
    <button class="mobile-nav-btn" onclick="switchPage('dashboard',null);setMobileActive(this)"><span>📊</span>Stats</button>
  </div>
</div>


<div id="addBookingModal" class="hidden" style="position:fixed;inset:0;background:rgba(0,0,0,.4);display:flex;justify-content:center;align-items:center;z-index:200">
  <div style="background:white;border-radius:20px;padding:32px;width:420px;box-shadow:0 24px 80px rgba(0,0,0,.2)">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px">
      <h2 style="font-size:18px;font-weight:800;color:#0F1B2D">Nouvelle reservation</h2>
      <button onclick="hideAddBooking()" style="background:none;border:none;font-size:20px;cursor:pointer;color:#94A3B8">✕</button>
    </div>
    <div class="stat-label">NOM</div>
    <input class="msg-input" type="text" id="nb-name" placeholder="Nom du client" style="margin-bottom:10px">
    <div class="stat-label">TELEPHONE</div>
    <input class="msg-input" type="text" id="nb-phone" placeholder="06 XX XX XX XX" style="margin-bottom:10px">
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
      <div><div class="stat-label">HEURE</div><input class="msg-input" type="text" id="nb-time" placeholder="20:30"></div>
      <div><div class="stat-label">COUVERTS</div><input class="msg-input" type="number" id="nb-covers" value="2" min="1" max="20"></div>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px">
      <div><div class="stat-label">SOURCE</div>
        <select id="nb-source" style="width:100%;padding:11px 14px;border-radius:10px;background:#F8FAFC;border:1.5px solid #E2E8F0;color:#0F1B2D;font-size:13px;font-family:inherit">
          <option value="phone">Telephone</option><option value="walkin">Walk-in</option><option value="whatsapp">WhatsApp</option><option value="zenchef">Zenchef</option>
        </select></div>
      <div><div class="stat-label">ZONE</div>
        <select id="nb-zone" style="width:100%;padding:11px 14px;border-radius:10px;background:#F8FAFC;border:1.5px solid #E2E8F0;color:#0F1B2D;font-size:13px;font-family:inherit">
          <option value="">Pas de preference</option><option value="salle">Salle</option><option value="terrasse">Terrasse</option><option value="bar">Bar</option>
        </select></div>
    </div>
    <div class="stat-label">NOTES</div>
    <input class="msg-input" type="text" id="nb-notes" placeholder="Allergie, occasion speciale..." style="margin-bottom:16px">
    <button onclick="submitManualBooking()" style="width:100%;padding:13px;border-radius:10px;border:none;background:#00D4AA;color:white;font-size:14px;font-weight:700;cursor:pointer;font-family:inherit">✅ Confirmer la reservation</button>
  </div>
</div>
...
const titles={floorplan:"Plan de salle",bookings:"Reservations",conversations:"Conversations",reviews:"Avis Google",contacts:"Contacts",config:"Configuration",dashboard:"Statistiques"};

function showAddBooking(){document.getElementById('addBookingModal').classList.remove('hidden');document.getElementById('addBookingModal').style.display='flex';}
function hideAddBooking(){document.getElementById('addBookingModal').classList.add('hidden');document.getElementById('addBookingModal').style.display='none';}
async function submitManualBooking(){
  var d={name:document.getElementById('nb-name').value,phone:document.getElementById('nb-phone').value,time:document.getElementById('nb-time').value,covers:document.getElementById('nb-covers').value,source:document.getElementById('nb-source').value,zone:document.getElementById('nb-zone').value,notes:document.getElementById('nb-notes').value};
  if(!d.name||!d.time){showToast('Nom et heure requis');return;}
  var r=await fetch(BASE+'/api/bookings/add?key='+SECRET,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});
  var j=await r.json();
  hideAddBooking();showToast('✅ Resa confirmee'+(j.table?' → '+j.table:''));
  document.getElementById('nb-name').value='';document.getElementById('nb-phone').value='';document.getElementById('nb-time').value='';document.getElementById('nb-notes').value='';document.getElementById('nb-covers').value='2';
  fetchFloorplan();fetchAllBookings();
}


function updateWidgetCode(){
  var color=document.getElementById('cfg-widget-color').value;
  var code='<script src="'+BASE+'/widget.js?color='+encodeURIComponent(color)+'"></'+'script>';
  document.getElementById('cfg-widget-code').value=code;
  document.getElementById('cfg-widget-preview').href=BASE+'/widget-preview?color='+encodeURIComponent(color);
}
function copyWidgetCode(){
  var el=document.getElementById('cfg-widget-code');
  el.select();
...
async function fetchConfig(){
  try{var r=await fetch(BASE+'/api/config?key='+SECRET);if(r.status===403)return;var d=await r.json();
  ...
  document.getElementById('cfg-hours').value=d.hours||'';
  document.getElementById('cfg-description').value=d.description||'';
  document.getElementById('cfg-tone').value=d.tone||'';
  document.getElementById('cfg-languages').value=d.languages||'';
  document.getElementById('cfg-special').value=d.special_info||'';
  document.getElementById('cfg-menu').value=d.menu||'';
  document.getElementById('cfg-allergens').value=d.allergens_policy||'';
  updateWidgetCode();
  }catch(e){console.error(e);}
  try{var r2=await fetch(BASE+'/api/settings?key='+SECRET);if(r2.status===403)return;var s=await r2.json();
  var pages=s.pages||{};var el=document.getElementById('cfgPages');
  var items=[['floorplan','🗺️ Plan de salle'],['bookings','📋 Reservations'],['conversations','💬 Conversations'],['reviews','⭐ Avis Google'],['contacts','👥 Contacts'],['dashboard','📊 Statistiques']];
  el.innerHTML=items.map(function(p){var on=pages[p[0]]!==false;return '<label style="display:flex;align-items:center;gap:10px;padding:10px;background:'+(on?'rgba(0,212,170,.06)':'#F8FAFC')+';border-radius:8px;cursor:pointer;border:1.5px solid '+(on?'rgba(0,212,170,.3)':'#E2E8F0')+'"><input type="checkbox" data-page="'+p[0]+'" '+(on?'checked':'')+' style="width:16px;height:16px;accent-color:#00D4AA"> <span style="font-size:13px;font-weight:600;color:#0F1B2D">'+p[1]+'</span></label>';}).join('');
  }catch(e){console.error(e);}
}

async function saveConfig(){
  var d={name:document.getElementById('cfg-name').value,address:document.getElementById('cfg-address').value,phone:document.getElementById('cfg-phone').value,hours:document.getElementById('cfg-hours').value,description:document.getElementById('cfg-description').value,tone:document.getElementById('cfg-tone').value,languages:document.getElementById('cfg-languages').value,special_info:document.getElementById('cfg-special').value,menu:document.getElementById('cfg-menu').value,allergens_policy:document.getElementById('cfg-allergens').value};
  await fetch(BASE+'/api/config?key='+SECRET,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});
  var pages={};document.querySelectorAll('#cfgPages input[type=checkbox]').forEach(function(cb){pages[cb.dataset.page]=cb.checked;});
  await fetch(BASE+'/api/settings?key='+SECRET,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({pages:pages})});
  showToast('💾 Configuration enregistree');
}

function switchPage(id,btn){document.querySelectorAll('.page').forEach(p=>p.classList.remove('active'));document.getElementById('page-'+id).classList.add('active');document.querySelectorAll('.nav-item').forEach(b=>b.classList.remove('active'));if(btn)btn.classList.add('active');document.getElementById('pageTitle').textContent=titles[id]||id;if(id==='conversations')fetchConversations();if(id==='reviews')fetchReviews();if(id==='contacts')fetchContacts();if(id==='config')fetchConfig();}
...
function renderSlotSelector(){
  ...
  el.innerHTML=slots.map(s=>{
    ...
    return '<button class="slot-btn'+(active?' active':'')+(count&&!active?' has-bookings':'')+'" data-slot="'+s+'" style="'+(active?'background:#0F1B2D;color:white':'')+'">'+s+(count&&!active?'<span class="slot-badge">'+count+'</span>':'')+'</button>';
  }).join('');
}
...
function renderFloorplan(){
  ...
  tables.forEach(t=>{
    ...
    tables_html+='<div class="'+cls+'" style="left:'+t.x+'%;top:'+t.y+'%;width:'+w+'px;height:'+h+'px;border-radius:'+br+';'+(booking?'border-color:'+srcColor+'60;background:'+srcColor+'10':'')+'" data-tid="'+t.id+'" >';
...
document.addEventListener('click',function(e){
  ...
  if(btn){curSlot=btn.dataset.slot;renderFloorplan();return;}
  var tbl=e.target.closest('[data-tid]');
  if(tbl){onTableClick(tbl.dataset.tid);return;}
  var act=e.target.closest('[data-action]');
  if(act){
    var bid=act.dataset.bid;
    if(act.dataset.action==='assign')startAssign(bid);
    else if(act.dataset.action==='change')startAssign(bid,true);
    else if(act.dataset.action==='release')releaseT(bid);
  }
});
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 RestoBot v4.0 démarré")
    import asyncio
...
async def webchat_message(request: Request):
    ...
    if session.get("name"):
        track_contact(f"web_{session_id[:8]}", session["name"])
...
(function(){
  ...
  style.textContent=`
    ...
    .rb-msg-bot{background:#F1F5F9;color:#0F1B2D;align-self:flex-start;border-bottom-left-radius:4px}
    .rb-msg-user{background:${COLOR};color:white;align-self:flex-end;border-bottom-right-radius:4px}
    .rb-typing{align-self:flex-start;background:#F1F5F9;padding:10px 16px;border-radius:14px;font-size:13px;color:#94A3B8}
    #rb-input-area{padding:12px;border-top:1px solid #E2E8F0;display:flex;gap:8px}
    #rb-input{flex:1;padding:10px 14px;border-radius:10px;border:1.5px solid #E2E8F0;font-size:13px;outline:none;font-family:inherit}
...
async def serve_widget(request: Request):
    ...
    color = request.query_params.get("color", "#00D4AA")
...
<style>body{{font-family:Inter,-apple-system,sans-serif;background:#F1F5F9;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0}}
.preview-box{{text-align:center;color:#64748B}}.preview-box h1{{color:#0F1B2D;font-size:24px;margin-bottom:8px}}.preview-box p{{font-size:14px}}</style>
</head><body>
