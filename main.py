...
function renderKPIs(d){
  var h='';
  h+=kpi(d.mrr+'\u20ac','MRR','var(--ok)');
  h+=kpi(d.arr+'\u20ac','ARR','var(--ok)');
  h+=kpi(d.active_count,'Actifs','var(--ok)');
  h+=kpi(d.trial_count,'En essai','var(--wa)');
  h+=kpi(d.conversion_rate+'%','Conversion','var(--ac)');
  h+=kpi(d.churn_rate+'%','Churn','var(--da)');
  h+=kpi(d.total_bookings,'Resas','var(--ac)');
