...
def build_availability_context(rid: str) -> str:
    ...
    lines.append("- Avant TOUTE confirmation de réservation, vérifie impérativement les HORAIRES D'OUVERTURE ci-dessus.")
    lines.append("- Si le restaurant est FERMÉ le jour demandé (voir HORAIRES D'OUVERTURE), REFUSE poliment et propose un jour ouvert.")
    lines.append("- Si le restaurant est OUVERT le jour demandé, vérifie que l'heure demandée tombe DANS une plage de service déclarée (ex: \"Déjeuner 12:00-14:30\", \"Brunch italien 12:00-15:00\"). Si l'heure est HORS des plages de service, REFUSE et propose une heure dans une plage ouverte.")
    lines.append("- Ne JAMAIS inventer d'horaires. Ne JAMAIS proposer un créneau en dehors des services affichés dans les HORAIRES D'OUVERTURE.")
    lines.append("- Les DISPONIBILITÉS en temps réel ci-dessus concernent UNIQUEMENT aujourd'hui.")
    ...
    lines.append("- Si un créneau est dispo dans la zone souhaitée ET dans un service ouvert, confirme la réservation en précisant le créneau et la date.")
    lines.append("- NE JAMAIS mentionner les numéros de table au client. Dis simplement que la réservation est confirmée.")
    lines.append("- NE JAMAIS confirmer une réservation si aucun créneau n'est disponible pour la zone et la période demandées, OU si le restaurant est FERMÉ ce jour-là.")
    return "\n".join(lines)
...
# Twilio + AI management routes extracted to app/routes/twilio_ai_routes.py
from app.routes.twilio_ai_routes import router as twilio_ai_router
app.include_router(twilio_ai_router)
...
# Dead booking/waitlist copies removed (already in booking_routes.py)
...
# Admin routes extracted to app/routes/admin_routes.py
