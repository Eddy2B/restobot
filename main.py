...
def build_availability_context(rid: str) -> str:
    ...
    # Per-zone availability for popular evening slot (20:00)
    zones = {}
    for t in tables:
        z = t.get("zone", "salle")
        zones.setdefault(z, {"total": 0, "available": 0})
        zones[z]["total"] += 1
    for slot_time in ["20:00", "20:30", "21:00"]:
        slot_data = table_slots.get(rid, {}).get(slot_time, {})
        for t in tables:
            z = t.get("zone", "salle")
            if slot_data.get(t["id"]) == "available":
                zones[z]["available"] = zones[z].get("available", 0)
    # Reset and compute for 20:00 specifically
    zone_avail_20 = {}
    slot_20 = table_slots.get(rid, {}).get("20:00", {})
    for t in tables:
        z = t.get("zone", "salle")
        zone_avail_20.setdefault(z, {"total": 0, "free": 0})
        zone_avail_20[z]["total"] += 1
        if slot_20.get(t["id"]) == "available":
            zone_avail_20[z]["free"] += 1

    lines.append("")
    lines.append("DÉTAIL PAR ZONE (créneau 20h00) :")
    for z, info in zone_avail_20.items():
        status = "COMPLET" if info["free"] == 0 else f"{info['free']} tables libres sur {info['total']}"
        lines.append(f"  - {z.upper()} : {status}")
