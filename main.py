...
<div class="lo" id="loginOverlay">
  <div class="lbox">
    ...
    <div class="lcd">
      ...
      <input class="linp" id="loginSecret" type="password" placeholder="Admin secret" autocomplete="off" style="margin-bottom:12px">
...
function renderTable(){
  ...
  restaurants.forEach(function(r){
    ...
    h+='<button class="btn btn-sm btn-ghost" onclick="openDetail(\''+r.id+'\')">Details</button>';
    h+='<button class="btn btn-sm btn-danger" onclick="confirmDelete(\''+r.id+'\',\''+esc(r.name)+'\')">Suppr.</button>';
    h+='</div></td>';
    h+='</tr>';
  });
...
function openDetail(rid){
  ...
  apiFetch('/api/admin/restaurant/'+rid).then(function(r){return r.json()}).then(function(d){
    ...
    statuses.forEach(function(s){
      ...
      h+='<button class="btn btn-sm '+cls+'" onclick="setStatus(\''+rid+'\',\''+s+'\')">'+label+'</button>';
    });
    h+='</div>';
    // KPIs
