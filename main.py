...
<div class="modal">
  ...
  <div class="modal-act"><button class="mbtn mbtn-s" onclick="deleteResa()" style="color:#DC2626">Supprimer</button><button class="mbtn mbtn-s" onclick="closeEditResa()">Annuler</button><button class="mbtn mbtn-p" onclick="saveEditResa()">Enregistrer</button></div>
</div>
</div>
...
function renderFloorplan(c){
  ...
  var zones=fpZones||[{id:'salle',label:'Salle',color:'#2563EB'},{id:'terrasse',label:'Terrasse',color:'#16A34A'},{id:'bar',label:'Bar',color:'#D97706'}];
  
  var h='<div class="card" style="padding:20px;margin-bottom:16px">';
  h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div><div class="card-t">Plan de salle</div><div class="card-s" id="fpSummary">'+floorplan.length+' tables  '+totalSeats+' places</div></div><div style="display:flex;gap:8px"><button class="ba" data-fpSave>Enregistrer</button></div></div>';
  
  // Zone tabs
  h+='<div style="display:flex;gap:6px;margin-bottom:10px">';
  zones.forEach(function(z){
    var cnt=floorplan.filter(function(t){return t.zone===z.id}).length;
    h+='<button style="padding:7px 14px;border-radius:8px;border:none;background:'+z.color+'15;color:'+z.color+';font-size:12px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpZone="'+z.id+'">'+z.label+' ('+cnt+')</button>';
  });
  h+='</div>';
  
  // Add table presets
  h+='<div style="display:flex;gap:5px;margin-bottom:10px;padding:8px 12px;background:var(--bg);border-radius:10px;overflow-x:auto"><span style="font-size:11px;font-weight:700;color:var(--tm);white-space:nowrap;display:flex;align-items:center;margin-right:4px">+ Ajouter :</span>';
  ...
  // Canvas
  h+='<div style="position:relative;height:420px;background:var(--bg);border-radius:14px;border:2px solid var(--b);overflow:hidden;touch-action:none;user-select:none" id="fpCanvas">';
  zones.forEach(function(z,i){
    ...
    h+='<div style="position:absolute;top:10px;left:50%;transform:translateX(-50%);font-size:10px;color:var(--tm);font-weight:700;letter-spacing:.04em;white-space:nowrap">'+z.label.toUpperCase()+'</div>';
    if(i<zones.length-1) h+='<div style="position:absolute;right:0;top:0;bottom:0;width:1px;border-right:1px dashed var(--b)"></div>';
    h+='</div>';
  });
  h+='</div>';
  ...
  h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px"><div style="font-size:15px;font-weight:700" id="fpEdTitle">Table</div><button style="padding:4px 10px;border-radius:6px;border:none;background:#DC262615;color:#DC2626;font-size:11px;font-weight:700;cursor:pointer;font-family:var(--f)" data-fpDel>Supprimer</button></div>';
...
var fpSelected=null;
var fpDragging=null;
var fpZones=[{id:'salle',label:'Salle',color:'#2563EB'},{id:'terrasse',label:'Terrasse',color:'#16A34A'},{id:'bar',label:'Bar',color:'#D97706'}];

function fpDrawTables(){
  var el=document.getElementById('fpCanvas');
  if(!el)return;
  el.querySelectorAll('.ftbl').forEach(function(e){e.remove()});
  var zc={salle:'#2563EB',terrasse:'#16A34A',bar:'#D97706'};
  floorplan.forEach(function(t,i){
    ...
    var co=zc[t.zone]||'#2563EB';
    var sel=fpSelected===i;
    var bk=t.booking_name;
    d.style.cssText='left:calc('+t.x+'% - '+w/2+'px);top:calc('+t.y+'% - '+h2/2+'px);width:'+w+'px;height:'+h2+'px;border-radius:'+(t.shape==='round'?'50%':'10px')+';border-color:'+(sel?co:bk?'#DC262660':co+'50')+';background:'+(sel?co+'20':bk?'#DC262610':co+'08')+';color:'+(bk?'#DC2626':co)+';cursor:grab;border-width:'+(sel?'2.5px':'2px')+';box-shadow:'+(sel?'0 4px 12px '+co+'30':'none')+';z-index:'+(sel?10:1)+';transition:box-shadow .15s';
...
function fpInitDrag(){
  ...
  function detectZone(x){
    if(x<46)return 'salle';
    if(x<84)return 'terrasse';
    return 'bar';
  }
  ...
  canvas.addEventListener('mousedown',function(e){
    ...
    if(tbl){
      e.preventDefault();
      fpDragging=parseInt(tbl.getAttribute('data-fpTbl'));
      fpSelected=fpDragging;
      fpShowEditor();
      fpDrawTables();
    }
  });
  ...
  function onMove(e){
    if(fpDragging===null)return;
    e.preventDefault();
    var p=getPos(e.touches?e.touches[0]:e);
    var t=floorplan[fpDragging];
    if(t){t.x=p.x;t.y=p.y;t.zone=detectZone(p.x);fpDrawTables()}
  }
  
  function onUp(){
    if(fpDragging!==null){fpShowEditor();fpDragging=null}
  }
...
function fpShowEditor(){
  ...
  if(fpSelected===null||!floorplan[fpSelected]){if(ed)ed.style.display='none';return}
...
function fpUpdateSummary(){
  ...
  if(el)el.textContent=floorplan.length+' tables  '+floorplan.reduce(function(a,t){return a+(t.seats||0)},0)+' places';
}

function fpSave(){
  var pid=Object.keys(restaurants||{})[0]||'';
  fetch('/api/config?key='+SK,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({tables:floorplan})}).then(function(){
    showToast('Plan de salle enregistre');
    // Also refresh mini
    drawFloorMini();
  });
}
